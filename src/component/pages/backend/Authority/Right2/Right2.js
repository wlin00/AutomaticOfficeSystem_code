...
import { Spin,Typography, Table, Input, Tag, Icon, Modal, Select, Button, message } from 'antd'
import RightContext from '../../../../../context/right/rightContext'
import Spinner from '../../../../laout/Spinner'
import qs from 'qs'
import axios from 'axios'

import '../Right2/Right2.scss'
...
const Right2 = (e) => {
    ...
    const { Search } = Input
    const { Option } = Select
    const addType = '费用审批' //要添加的人员的权限类型
    ...
    const handleOk2 = e => {
        setVisible2(false)
        console.log(delName)
        axios.delete(`/AutomaticOfficeSystem/approvalManagement/api/v1/approvalUser?permission=${'2'}&userUsername=${delName}`)
            .then((data) => {
         ...
         <div className='Right2'>
            <div>
                <Modal
                    ...
                    destroyOnClose={true}
                >
                    <div className='Right2-wrap__div'>
                        <Text style={{ fontSize: 'normal' }} className="Right2-wrap__text">职务</Text>
                        <Input
                            className="Right2-wrap__input"
                            placeholder="请输入审批权限"
                            value={addType}
                            disabled={true}
                    ...
                    <div className='Right2-wrap__div'>
                        <Text style={{ fontSize: 'normal' }} className="Right2-wrap__text">用户名</Text>
                        <Input
                            className="Right2-wrap__input"
                            placeholder="请输入用户名"
                            value={username}
            ...
            <ul className='Right2-ul'>
                <li className='Right2-ul__li Right2-ul__li--posi'   >
                    <Button onClick={showModal} style={{ border: "none" }}>
                        <Icon className='Right2-ul__li--icon' type="plus-square" />
                        <Text className='addText' style={{ position: 'relative', left: '5px' }}>添加成员</Text>
                    </Button>
                </li>

            </ul>
            <div className='Right2-wrap'>
...
export default React.memo(Right2)
